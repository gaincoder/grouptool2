...
class AnswerEvent implements BotControllerInterface
{
    ...
    {
        $eventRepo = $this->entityManager->getRepository('App:Event');
        $event = $eventRepo->find($this->data->eventId);
        $voteRepo = $this->entityManager->getRepository('App:EventVote');
        ...
        if ($event->owner instanceof UserInterface && $event->owner->telegramSupported()) {
            ...
            $message = $this->user->getUsername() . " hat bei deiner Aktivität ";
